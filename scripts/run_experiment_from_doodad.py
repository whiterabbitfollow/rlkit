...
if __name__ == "__main__":
    ...
    if run_mode and run_mode in ['slurm_singularity', 'sss']:
        import os
        run_experiment_kwargs['variant']['slurm-job-id'] = os.environ.get('SLURM_JOB_ID', None)
    if run_mode and (run_mode == 'ec2' or run_mode == 'gcp'):
        ...
        if run_mode == 'gcp':
            try:
                ...
                run_experiment_kwargs['variant']['GCP_instance_name'] = (instance_name)
            except Exception as e:
        ...
        run_experiment_here(method_call, include_exp_prefix_sub_dir=False, **run_experiment_kwargs)
    else:
